...
for k=1:length(index1)
    [idx1, idx1] = min(abs(displacements(:,k)-1.9));
    [idx2, idx2] = min(abs(displacements(:,k)-3.9));
    index1(k) = idx1;
    index2(k) = idx2;
end
figure
...
t = timestep*(2000:7000);

vel1 = polyfit(t,index1(2000:7000),1);
vel2 = polyfit(t,index2(2000:7000),1);
